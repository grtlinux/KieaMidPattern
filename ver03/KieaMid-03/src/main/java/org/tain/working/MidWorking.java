...
public class MidWorking {
		...
		if (Flag.flag) {
			this.compRepository.getAll().forEach(entity -> {
				DivisionComp divisionComp = DivisionComp.builder()
						...
						.build();
				if (Flag.flag) System.out.println("getAll >>>>> json: " + divisionComp.toPrettyJson());
			});
		}
	}
	...
	public void getPage() {
		...
		if (Flag.flag) {
			...
			this.compRepository.getPage(pageable).forEach(entity -> {
				DivisionComp divisionComp = DivisionComp.builder()
						.divCode(entity.getDivCode())
						.divTitle(entity.getDivTitle())
						.divContent(entity.getDivContent())
						...
						.compStmtEn(entity.getCompStmtEn())
						.build();
				if (Flag.flag) System.out.println("getPage >>>>> json: " + divisionComp.toPrettyJson());
			});
		}
	}
